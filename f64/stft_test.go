...
// TestISTFTShortDst exercises the base>=n early break: a dst shorter than the
...
func TestISTFTShortDst(t *testing.T) {
	const nfft, hop = 64, 16
	p, _ := NewSTFTPlan(nfft)
	x := testSignal(512)
	frames := p.NumFrames(len(x), hop, NoPad)
