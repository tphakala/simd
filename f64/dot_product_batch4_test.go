...
func TestDotProductBatchEmptyVec(t *testing.T) {
	rows := [][]float64{
		deterministicF64Vector(1, 10),
		deterministicF64Vector(2, 20),
		nil,
		deterministicF64Vector(3, 5),
	}
	for _, vec := range [][]float64{nil, {}} {
		results := []float64{111, 222, 333, 444, 555} // one extra to pin results[:n]
