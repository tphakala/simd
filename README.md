...
[![codecov](https://codecov.io/gh/tphakala/simd/branch/main/graph/badge.svg)](https://codecov.io/gh/tphakala/simd)
...
`RealFFTUnpack` and writes a full frame with `Scale` and `Interleave2`, and
`ISTFT` overlap-adds with `MulAdd` (`AccumulateAdd` for a rectangular window) and
normalizes its interior with `Div`. Like `STFT`, the inverse output is
tolerance-stable, not bit-stable, across CPU tiers and library versions.
...
|                | `SumSqShiftedQ31(a, shift) int32` | Wrapping int32 sum of `MULT32_32_Q31(a[i]<<shift, a[i]<<shift)`, the libopus `compute_band_energies` band-energy reduction | 8x (AVX2) / 4x (NEON) |
...
|                 | `FIRSymValidQ15(dst, x, center, pairs)` | Symmetric valid convolution (center tap + K mirror pairs), each pair folded before a single Q15 truncation, wrapping accumulate | 8x (AVX2) / 4x (NEON) |
