...
func BenchmarkISTFT(b *testing.B) {
	...
	for _, bc := range []struct {
		...
		{"hann/NoPad", hannF32(nfft), NoPad},
		{"hann/PadZero", hannF32(nfft), PadZero},
		{"rect/NoPad", nil, NoPad},
	} {
		b.Run(bc.name, func(b *testing.B) {
			plan, _ := NewSTFTPlan(nfft)
			spec := make([][]complex64, plan.NumFrames(len(signal), hop, bc.pad))
			for f := range spec {
				spec[f] = make([]complex64, plan.NumBins())
			}
			plan.STFT(spec, signal, bc.window, hop, bc.pad)
			dst := make([]float32, len(signal)+nfft)
			b.ReportAllocs()
			for b.Loop() {
...
func BenchmarkISTFTHopSweep(b *testing.B) {
	...
	for _, hop := range []int{1, 4, 16, nfft / 4} {
		b.Run(fmt.Sprintf("hop%d", hop), func(b *testing.B) {
			plan, _ := NewSTFTPlan(nfft)
			spec := make([][]complex64, plan.NumFrames(len(signal), hop, PadZero))
			for f := range spec {
				spec[f] = make([]complex64, plan.NumBins())
			}
...
func TestIRFFTShortInputsF32(t *testing.T) {
	...
	for _, nfft := range []int{4, 8, 16, 32} {
		t.Run(fmt.Sprintf("nfft%d", nfft), func(t *testing.T) {
			...
			zeros := make([]complex64, p.NumBins())
			p.IRFFT(want, zeros)
			p.IRFFT(got, spec)
			for i := range got {
				got[i] = 1
...
// TestISTFTHugeHopF32 pins the #294 fix: a hop so large that the frame and
...
func TestISTFTHugeHopF32(t *testing.T) {
	p, _ := NewSTFTPlan(4)
	spec := [][]complex64{{1, 2, 3}, {1, 2, 3}}
	win := []float32{0, 0.5, 1, 0.5}
	for _, hop := range []int{math.MaxInt, math.MaxInt - 50} {
		for _, pad := range []PadMode{NoPad, PadZero, PadReflect} {
			for _, w := range [][]float32{win, nil} {
				got := p.ISTFT(make([]float32, 100), spec, w, hop, pad)
				if got != 0 {
	...
	one := [][]complex64{{1, 2, 3}}
	if got := p.ISTFT(make([]float32, 100), one, win, math.MaxInt, NoPad); got != p.NFFT() {
...
// TestISTFTLibrosaParityF32 pins ISTFT's normalization and centering against
...
// this package's own STFT (already pinned to librosa by TestSTFTLibrosaParityF32).
func TestISTFTLibrosaParityF32(t *testing.T) {
	var g struct {
		LibrosaVersion string  `json:"librosa_version"`
		NFFT           int     `json:"nfft"`
		Hop            int     `json:"hop"`
	...
	window := hannF32(g.NFFT)
	p, _ := NewSTFTPlan(g.NFFT)
	gain := make([]float32, p.NumBins())
	for k := range gain {
		gain[k] = float32(0.5 + 0.5*math.Cos(float64(k)/g.GainPeriod))
	}
	padOf := map[string]PadMode{"PadZero": PadZero, "PadReflect": PadReflect}
	for _, c := range g.Cases {
		pad := padOf[c.GoPad]
		...
		var maxErr float64
		for i := range y {
			maxErr = max(maxErr, math.Abs(float64(y[i])-c.Y[i]))
		}
		// float32 forward + inverse against librosa's float64: the observed
		// error is ~1e-6 of peak; 1e-4 keeps margin while any convention slip
		// (missing normalization, wrong trim) is orders of magnitude larger.
		if maxErr > 1e-4*peak {
			t.Errorf("%s: max abs error %g exceeds 1e-4 of peak %g vs librosa", c.GoPad, maxErr, peak)
		}
	}
}
...
// TestISTFTWindowedShortDstPrefixF32 pins that a windowed ISTFT into a dst
// shorter than the full reconstruction is a bit-exact prefix of the full output,
// at every truncation length. Both the per-frame MulAdd span (hi = min(nfft,
// n-base)) and the block-normalization interior (uHi clamped to n+off) depend on
// n, so a length-dependent slip would move an early sample.
func TestISTFTWindowedShortDstPrefixF32(t *testing.T) {
	const nfft, hop = 64, 16
	x := testSignalF32(700)
	window := hannF32(nfft)
...
// TestISTFTOverlapAddReferenceF32 checks the overlap-add (MulAdd with a window,
...
func TestISTFTOverlapAddReferenceF32(t *testing.T) {
	const nfft = 128
	x := testSignalF32(1000)
	for _, window := range [][]float32{hannF32(nfft), nil} {
		for _, pad := range []PadMode{NoPad, PadZero, PadReflect} {
			for _, hop := range []int{nfft / 4, nfft / 2, 50, 150} {
				p, _ := NewSTFTPlan(nfft)
				spec := make([][]complex64, p.NumFrames(len(x), hop, pad))
				for f := range spec {
					spec[f] = make([]complex64, p.NumBins())
				}
				p.STFT(spec, x, window, hop, pad)
				...
				for _, dstLen := range []int{full, full - 37, nfft/2 + 3, hop - 1} {
					want := istftReferenceF32(p, spec, window, hop, pad, dstLen)
					y := make([]float32, dstLen)
					if n := p.ISTFT(y, spec, window, hop, pad); n != dstLen {
						t.Fatalf("window=%t pad=%v hop=%d len=%d: ISTFT wrote %d", window != nil, pad, hop, dstLen, n)
					}
					for i := range y {
						if d := math.Abs(float64(y[i]) - want[i]); d > 1e-5*(1+math.Abs(want[i])) {
...
// istftReferenceF32 is the per-sample scalar ISTFT the overlap-add reference test
// compares against: every frame sample is windowed and bounds-checked on its own,
// and the squared window is accumulated alongside, in float64.
func istftReferenceF32(p *STFTPlan, spec [][]complex64, window []float32, hop int, pad PadMode, n int) []float64 {
	off := 0
	if pad != NoPad {
		off = p.NFFT() / 2
	}
...
// TestNormalizeISTFTMatchesPerSampleF32 pins the block normalization bit for bit
...
func TestNormalizeISTFTMatchesPerSampleF32(t *testing.T) {
	const nfft = 64
	p, _ := NewSTFTPlan(nfft)
	src := testSignalF32(700)
	subFloorWin := append([]float32(nil), hannF32(nfft)...)
	subFloorWin[0] = 1e-5 // w^2 = 1e-10, strictly between 0 and istftNormFloor
	for _, window := range [][]float32{hannF32(nfft), subFloorWin, nil} {
...
func FuzzRFFTIRFFT(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, raw []byte, nfftSel, hopSel, selSel uint8, useWin bool) {
		...
		tol := stftTolF32(nfft, 2)
		for i := range nfft {
			if d := math.Abs(float64(y[i] - frame[i])); d > tol {
				t.Fatalf("nfft=%d sample %d: round trip %g != %g (|diff|=%g tol=%g)", nfft, i, y[i], frame[i], d, tol)
			}
		}

		// Invariant 2: IRFFT ignores imaginary DC and Nyquist. randomSpectrumF32
		// puts a nonzero imaginary part on every bin, including DC (index 0) and
		// Nyquist (bins-1); zeroing those imaginaries must not move the output.
		seed := 0.5 + float64(selSel)
		full := randomSpectrumF32(bins, seed)
		zeroed := make([]complex64, bins)
		copy(zeroed, full)
...
func FuzzISTFT(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, raw []byte, nfftSel, hopSel uint8, useWin bool, padSel uint8) {
		...
		spec := make([][]complex64, frames)
		for i := range spec {
			spec[i] = make([]complex64, plan.NumBins())
		}
		plan.STFT(spec, signal, window, hop, pad)

		y := make([]float32, len(signal))
		...
		for i := range n {
			...
			if d := math.Abs(float64(y[i] - signal[i])); d > tol {
