...
type STFTPlan struct {
	...
	// interleaved into the caller's complex64 row (index 0 unused there; DC and
...
func NewSTFTPlan(nfft int) (*STFTPlan, error) {
	...
	p := &STFTPlan{
		...
		altScale:    make([]float32, nfft),
		pw:          make([]float32, nfft),
	}
...
func sampleAt(signal []float32, idx int, pad PadMode) float32 {
